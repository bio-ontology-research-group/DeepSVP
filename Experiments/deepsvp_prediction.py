...
def preprocessing_numeric(features,cols):
...
def collect_features(data, onto, operation, ids, dl2vec, res_go,res_mp,res_hp,res_cl,res_ub):
    ...
    if (onto == 'all'):
        ...
        Gene_features['uberon_score'] = Gene_features.apply(lambda x: get_scores(res_ub,x['ID'], x['entrezgene']), axis=1)                                                                
        ...
        Gene_features[onto+'_score'] = Gene_features.apply(lambda x: get_scores(dl2vec,x['ID'], x['entrezgene']), axis=1)
    ...
    features = preprocessing_numeric(features,col)    
    return features
